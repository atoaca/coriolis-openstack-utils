...
class SecurityGroupCreationAction(Action):
    action_type = ACTION_TYPE_CHECK_CREATE_SECGROUP
...
class NetworkCreationAction(Action):
    action_type = ACTION_TYPE_CHECK_CREATE_NETWORK
    ...
    def equivalent_to(self, other_action):
        if other_action.action_type == self.action_type:
            if self.payload['src_network_id'] == (
                    other_action.payload.get('src_network_id')):
                return (self.payload.get('dest_tenant_id') ==
                        other_action.payload.get('dest_tenant_id'))
        return False
    ...
    def create_network_body(self, description):
        src_body = networks.get_body(
            self._source_client, self.payload['src_network_id'])
        dest_network_name = self.get_new_network_name()
        dest_tenant_id = self.payload['dest_tenant_id']
    ...
    def execute_operations(self):
        ...
        description = (self.NEW_NETWORK_DESCRIPTION %
                       self.get_source_network_name())

        body = self.create_network_body(description)
        # TODO: create subnets from destination
        ...
        for name in src_subnet_names:
            ...
            self.subactions.append(
                SubnetCreationAction(
                    self._source_client, self._destination_client,
                    subnet_migration_payload))

        for subaction in self.subactions:
            subaction.execute_operations()
